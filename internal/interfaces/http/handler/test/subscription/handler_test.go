...
func activeClient() fiber.Handler {
	return testutil.WithActiveClient(testutil.TestUserID, testutil.TestClientID)
}
...
func sampleQuotaUsageView() *querysubscription.QuotaUsageView {
	...
	return &querysubscription.QuotaUsageView{
		Members:   querysubscription.QuotaCounter{Used: 1, Max: 5, Left: 4},
		Campaigns: querysubscription.QuotaCounter{Used: 2, Max: 10, Left: 8},
		Verifications: querysubscription.MonthlyQuotaCounter{
			PeriodStart: now,
		...
		ConcurrentAnalyses: querysubscription.QuotaCounter{Used: 1, Max: 3, Left: 2},
		Limits: querysubscription.QuotaLimits{
			MaxFileSizeMB:       50,
			ReportRetentionDays: 30,
			AllowsVideoAnalysis: true,
			AllowsPDFExport:     true,
			AllowsCSVExport:     true,
...
func TestSubscriptionHandler_GetSubscription_Success(t *testing.T) {
	...
	app.Get("/subscriptions", activeClient(), h.GetSubscription)
	...
	if getCurrent.query.WorkspaceID != testutil.TestWorkspaceID {
		t.Fatalf("workspace id: got %s", getCurrent.query.WorkspaceID)
	}
...
func TestSubscriptionHandler_GetQuota_Success(t *testing.T) {
	...
	app := testutil.NewTestApp()
	app.Get("/subscription/quota", activeClient(), h.GetQuota)
	...
	if getQuota.query.ClientID != testutil.TestClientID {
		t.Fatalf("client id: got %s", getQuota.query.ClientID)
	}

	var out presenter.QuotaUsageResponse
	testutil.DecodeJSON(t, resp, &out)
	if out.Verifications.Used != 5 {
		t.Fatalf("verifications used: got %d want 5", out.Verifications.Used)
	}
	if out.Limits.MaxFileSizeMB != 50 {
		t.Fatalf("max file size: got %d want 50", out.Limits.MaxFileSizeMB)
	}
...
func TestSubscriptionHandler_GetQuota_MissingActiveProject(t *testing.T) {
	...
	app := testutil.NewTestApp()
	app.Get("/subscription/quota", testutil.WithUserWithoutClient(testutil.TestUserID), h.GetQuota)
...
func TestSubscriptionHandler_PreviewSubscription_Success(t *testing.T) {
	...
	app := testutil.NewTestApp()
	app.Post("/subscription/preview", activeClient(), h.PreviewSubscription)
	...
	if preview.query.WorkspaceID != testutil.TestWorkspaceID {
		t.Fatalf("workspace id: got %s", preview.query.WorkspaceID)
	}
...
func TestSubscriptionHandler_CreateSubscription_Success(t *testing.T) {
	...
	app := testutil.NewTestApp()
	app.Post("/subscription", activeClient(), h.CreateSubscription)
	...
	if create.cmd.WorkspaceID != testutil.TestWorkspaceID {
		t.Fatalf("workspace id: got %s", create.cmd.WorkspaceID)
	}
...
func TestSubscriptionHandler_CreateBillingPortal_Success(t *testing.T) {
	...
	app := testutil.NewTestApp()
	app.Post("/subscription/billing-portal", activeClient(), h.CreateBillingPortal)
	...
	if billingPortal.cmd.WorkspaceID != testutil.TestWorkspaceID {
		t.Fatalf("workspace id: got %s", billingPortal.cmd.WorkspaceID)
	}
...
func TestSubscriptionHandler_CreateSubscription_Forbidden_NotOwner(t *testing.T) {
	create := &mockCreateSubscriptionHandler{err: cmdsubscription.ErrNotWorkspaceOwner}
	h := newSubscriptionHandler(nil, nil, nil, create, nil)
...
func TestSubscriptionHandler_CreateBillingPortal_Forbidden_NotOwner(t *testing.T) {
	billingPortal := &mockCreateBillingPortalHandler{err: cmdsubscription.ErrNotWorkspaceOwner}
	h := newSubscriptionHandler(nil, nil, nil, nil, billingPortal)
