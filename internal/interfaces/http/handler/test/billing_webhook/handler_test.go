...
func TestBillingWebhookHandler_Execute_CheckoutCompleted_Success(t *testing.T) {
	...
	event := stripeEvent("checkout.session.completed", map[string]any{
		"client_reference_id": testutil.TestWorkspaceID.String(),
	...
	if checkout.cmd.WorkspaceID != testutil.TestWorkspaceID {
		t.Fatalf("workspace id: got %s", checkout.cmd.WorkspaceID)
	}
...
func TestBillingWebhookHandler_Execute_MissingPayload(t *testing.T) {
	h := newBillingWebhookHandler(billingWebhookMocks{})
	app := testutil.NewTestApp()
	app.Post("/webhooks/billing", h.Execute)
