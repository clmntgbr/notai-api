...
type Config struct {
	...
	SchedulerStaleMaxAge       time.Duration
}
...
func Load() *Config {
	...
	return &Config{
		...
		SchedulerStaleAfter:        getEnvDuration("SCHEDULER_STALE_AFTER", 30*time.Minute),
		SchedulerStaleMaxAge:       getEnvDuration("SCHEDULER_STALE_MAX_AGE", 2*time.Hour),
	}
}
