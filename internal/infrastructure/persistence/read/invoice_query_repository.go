...
func (r *invoiceReadRepository) FindByWorkspaceID(
	ctx context.Context,
	...
	case "total", "status", "created_at", "amount_due", "amount_paid":
		// keep whitelisted column
	default:
		query.SortBy = "stripe_created_at"
	}
	...
	db := r.db.WithContext(ctx).Model(&invoiceRow{}).Where("workspace_id = ?", workspaceID)
